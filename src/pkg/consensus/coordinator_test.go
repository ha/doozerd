...
func TestCoordIgnoreOldMessages(t *testing.T) {
	...
	co.update(packet{msg: *newPropose("foo")})

	co.update(packet{msg: *msgTick}) // force the start of a new round
...
func TestCoordStart(t *testing.T) {
	...
	got, tick := co.update(packet{msg: *newPropose("foo")})
...
func TestCoordRetry(t *testing.T) {
	...
	got, tick = co.update(packet{msg: *msgTick}) // force the start of a new round
...
func TestCoordinatorIgnoresBadMessages(t *testing.T) {
	...
	// missing Vrnd
	got, tick = co.update(packet{msg: msg{Cmd: rsvp, Crnd: new(int64)}})
	assert.Equal(t, (*msg)(nil), got)
	assert.Equal(t, false, tick)
	assert.Equal(t, coordinator{begun: true}, co)
}
